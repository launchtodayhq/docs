...
- `apps/api/.env` created from `apps/api/example.env`

## Steps

1. Copy `apps/api/example.env` to `apps/api/.env` and set auth vars
2. Configure providers in `apps/api/src/services/auth.ts` (Apple/Google as needed)
3. Run migrations in `apps/api`
4. Start the API server with `pnpm dev`
...
- **OAuth callbacks fail**: check `BETTER_AUTH_URL`
- **Provider not available**: verify provider env vars
...
export const captureDeviceInfo = async (
  request: Request,
  sessionToken: string,
) => {
  const deviceInfo = {
