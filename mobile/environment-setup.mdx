...
Create an environment file in the API backend (`apps/api/.env`):
...
**API Backend** (`apps/api/.env`) — see the full list in
...
The mobile app config lives in `apps/mobile/app.config.ts`, and feature
toggles/providers are defined in `apps/mobile/features/feature-registry.tsx`.
...
ios: {
  ...
  bundleIdentifier: "com.launchhq.mobile", // Must match APPLE_CLIENT_ID
}
```
...
Set the API URL in your mobile env file:
...
The auth client (`apps/mobile/lib/auth/client.ts`) and tRPC client read
`EXPO_PUBLIC_API_URL` automatically.
...
- Set `EXPO_PUBLIC_API_URL` in `apps/mobile/.env` to production API
...
- **Auth failures**: verify `BETTER_AUTH_URL` and provider env vars
- **Expo build errors**: re-run with `pnpm start --clear`
- **Device can’t reach API**: use your LAN IP in `EXPO_PUBLIC_API_URL`
